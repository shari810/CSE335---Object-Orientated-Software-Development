...
void CityView::AddMenus(wxFrame* mainFrame, wxMenuBar *menuBar, wxMenu* fileMenu, wxMenu* viewMenu)
{
    ...
    businessesMenu->Append(IDM_BUESINESS_HAUL, L"&Haul", L"Count the total coal mined");
    mainFrame->Bind(wxEVT_COMMAND_MENU_SELECTED, &CityView::OnCoalmineHaul, this, IDM_BUESINESS_HAUL);

    businessesMenu->Append(IDM_BUSINESS_BOOST, L"&Boost", L"Boost coal production", wxITEM_CHECK);
    mainFrame->Bind(wxEVT_COMMAND_MENU_SELECTED, &CityView::OnBoost, this, IDM_BUSINESS_BOOST);
 ...
 * @param event Menu event
 */
void CityView::OnCoalmineHaul(wxCommandEvent& event)
{

    HaulCoalVisitor visitor;
    mCity.Accept(&visitor);
    int totalProduction = visitor.GetNumCoalHauled();
...
/**
 * Handle the Business->Boost menu option
 * @param event menu event
 */
