 ...
 */
FishMagikarp::FishMagikarp(Aquarium *aquarium) : Item(aquarium)
{
    mFishImage = make_unique<wxImage>(FishMagikarpImageName, wxBITMAP_TYPE_ANY);
 ...
 */
void FishMagikarp::Draw(wxDC *dc)
{
 ...
 * @return true if hit.
 */
bool FishMagikarp::HitTest(int x, int y)
{
    double wid = mFishBitmap->GetWidth();
    double hit = mFishBitmap->GetHeight();
